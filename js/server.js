...
function createComment(url, request) {
  const requestComment = request.body && request.body.article;
  const response = {};
...
//don't know how to get the articleId based on a comment
      articleId: [],
      commentIds: [],
      upvotedBy: [],
      downvotedBy: []
}
if (request.body && requestComment.userName && articleId) {response.status = 200} else {response.status = 400}
...
//new code to upvote the comments
...
//new code to downvote a comment
function downvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let savedComment = database.comment[id];
  const response = {};
